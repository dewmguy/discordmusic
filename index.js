...
//discord
const { Client, GatewayIntentBits, SlashCommandBuilder, EmbedBuilder, REST, Routes, MessageFlags, PermissionsBitField } = require("discord.js");
...
async function queueMusic(interaction, query) {
  ...
  if (!isURL(query)) {
    const directive = "you are a music expert. a user will provide query from a search function. this query could be gibberish, song lyrics, or a request to play a song. your objective is to guide, interpret, fix typos, correct common misconceptions, and provide output in the form of a song title and artist name (e.g. 'Homework Daft Punk'). Your output will be fed directly into another app for processing, no further output is required. Above all, trust the user, they may be asking for a song you haven't heard of.";
...
const commandHandlers = {
  music: async (interaction) => {
    ...
    await interaction.deferReply({ flags: subcommand === "queue" ? 0 : MessageFlags.Ephemeral });
...
client.on("interactionCreate", async (interaction) => {
  ...
  if (commandHandlers[commandName]) { await commandHandlers[commandName](interaction); }
});
